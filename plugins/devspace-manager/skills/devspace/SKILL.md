...
5. Use `task`/`delegate` when the user wants Codex to outsource work to ChatGPT with explicit control over `--send`. It starts and verifies DevSpace, adds a narrow exchange root under `~/.devspace/manager/exchange/<task>`, writes a ChatGPT-ready prompt under `~/.devspace/manager/tasks`, sends that prompt through DevSpace Manager's built-in strict-background ChatGPT sender with `--send chatgpt-app`, and waits for ChatGPT to write the result into the exchange root through DevSpace. If ChatGPT has no accessible hidden window, the sender must fail closed rather than showing a window. The file access path remains DevSpace MCP, not zip upload.
